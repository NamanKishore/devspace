  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-10 space-y-6">
        ...
        className="primary-gradient paragraph-medium min-h-2 w-full rounded-2 px-4 py-3 font-inter !text-light-900">
         {form.formState.isSubmitting ? buttonText === 'Sign In' ? 'Signing In...' : 'Signing Up...' : buttonText}
        </Button>
      {formType === 'SIGN_IN' ? <p>Dont have an Account ?{" "}<Link href={ROUTES.SIGNUP} className="paragraph-semibold primary-text-gradient">Sign Up</Link></p> : <p>Already have an account?{' '} <Link href={ROUTES.SIGNIN}>Sign In</Link></p>}
